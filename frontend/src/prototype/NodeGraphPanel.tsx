  ...
  return (
    <section className="node-panel panel" aria-label="Causal node graph">
      <div className="panel-copy">
        ...
        <p>Read the three highest-signal drivers first, then expand the full causal map when you need the wiring.</p>
      </div>
      <div
        style={{
          display: "grid",
      ...
      >
        <div
          ...
          aria-label="Causal graph highlights"
        >
          {highlights.map((highlight) => (
            ...
            >
              ...
              <strong style={{ fontSize: "1.15rem" }}>{highlight.value}</strong>
              <small style={{ color: "var(--muted)" }}>{highlight.note}</small>
            </article>
          ))}
        </div>
        <div
        ...
        >
          <p style={{ margin: 0, color: "var(--muted)" }}>The full graph is collapsed by default to keep the workspace compact.</p>
          <button
            type="button"
            aria-expanded={expanded}
            aria-controls="forest-causal-graph"
            onClick={() => setExpanded((current) => !current)}
          >
            {expanded ? "Collapse graph" : "Show full graph"}
          </button>
        </div>
      </div>
      <div hidden={!expanded} id="forest-causal-graph" aria-hidden={!expanded}>
        <svg className="node-graph" viewBox="0 0 720 420" role="img" aria-label="Forest systems graph">
          <defs>
          ...
          {nodes.map((node) => {
            ...
            return (
              <g key={node.id} className="graph-node" data-testid={`node-${node.id}`} transform={`translate(${node.x}, ${node.y})`}>
                <rect
                  width="136"
                  height="64"
                  rx="16"
